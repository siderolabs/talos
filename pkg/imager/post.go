...
func (i *Imager) postProcessGz(_ context.Context, filename string, report *reporter.Reporter) (string, error) {
	report.Report(reporter.Update{Message: "compressing .gz", Status: reporter.StatusRunning})

	if _, err := cmd.Run("pigz", "-6", "-f", filename); err != nil { //nolint:staticcheck
		return "", err
	}
