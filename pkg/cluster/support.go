...
// GetSupportBundle writes all cluster information we can gather into a single zip archive.
//nolint:gocyclo
func GetSupportBundle(ctx context.Context, options *BundleOptions) error {
	var err error
	...
	for _, dynamic := range []struct {
		...
		{"kubernetes resources", getKubernetesResources},
	} {
	...
	go func() {
		...
		for _, nodeCollector := range cols {
			...
			if options.Progress != nil {
				options.Progress <- BundleProgress{Node: options.Node, Value: done, Total: len(cols), State: strings.Split(line, "\n")[0]}
			}
		}
	}()
	...
	for i := 0; i < options.NumWorkers; i++ {
		...
		go func() {
			...
			for nodeCollector := range colChan {
				...
				if data, err = nodeCollector.collect(ctx, options); err != nil {
					options.Errorf("failed to get %s: %s, skipped", nodeCollector.filename, err)
				...
				if err = options.Archive.Write(fmt.Sprintf("%s/%s", options.Node, nodeCollector.filename), data); err != nil {
					return
				}
			}
		}()
...
func getKubernetesResources(ctx context.Context, c *client.Client) ([]nodeCollector, error) {
	...
	if _, err = clientset.CoreV1().Namespaces().Get(ctx, "kube-system", v1.GetOptions{}); err != nil {
		return nil, err
	}
