...
func (m *ImageServiceListRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Containerd != nil {
		size, err := m.Containerd.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *ImageServiceListRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if m.Containerd == nil {
				m.Containerd = &ContainerdInstance{}
			}
			if err := m.Containerd.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
	...
	for iNdEx < l {
		...
		case 1:
			...
			if oneof, ok := m.Request.(*ImageServiceImportRequest_Containerd); ok {
				if err := oneof.Containerd.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				}
			} else {
				v := &ContainerdInstance{}
				if err := v.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				}
