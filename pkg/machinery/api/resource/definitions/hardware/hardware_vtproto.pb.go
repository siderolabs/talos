...
func (m *PCIDriverRebindStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.OriginalBoundDriver) > 0 {
		i -= len(m.OriginalBoundDriver)
		copy(dAtA[i:], m.OriginalBoundDriver)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.OriginalBoundDriver)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Pciid) > 0 {
...
func (m *PCIDriverRebindStatusSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field OriginalBoundDriver", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.OriginalBoundDriver = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TargetDriver", wireType)
