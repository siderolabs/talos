...
func (m *LogicalCPUInfoSpec) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *LogicalCPUInfoSpec) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *LogicalCPUInfoSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.Core != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Core))
		i--
		dAtA[i] = 0x18
	}
	if m.Socket != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Socket))
		i--
		dAtA[i] = 0x10
	}
...
func (m *LogicalCPUInfoSpec) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if m.Socket != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.Socket))
	}
	if m.Core != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.Core))
	}
	if len(m.Bugs) > 0 {
	...
	if m.NumaNode != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.NumaNode))
	}
...
func (m *LogicalCPUInfoSpec) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		if wireType == 4 {
			return fmt.Errorf("proto: LogicalCPUInfoSpec: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: LogicalCPUInfoSpec: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 5:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NumaNode |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
