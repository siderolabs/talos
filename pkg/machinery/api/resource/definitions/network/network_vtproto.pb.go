...
func (m *LinkStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.PrivateFlags) > 0 {
		for k := range m.PrivateFlags {
			v := m.PrivateFlags[k]
			baseI := i
			i--
			if v {
...
func (m *LinkStatusSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 32:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PrivateFlags", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.PrivateFlags == nil {
				m.PrivateFlags = make(map[string]bool)
			}
			var mapkey string
			var mapvalue bool
			for iNdEx < postIndex {
			...
			m.PrivateFlags[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
