...
func (m *ConfigSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.EndpointFilters) > 0 {
		for iNdEx := len(m.EndpointFilters) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarint(dAtA, i, uint64(len(m.EndpointFilters[iNdEx])))
			i--
			dAtA[i] = 0x3a
		}
	}
	...
	if len(m.SharedSecret) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.SharedSecret)))
		i--
		dAtA[i] = 0x1a
	}
...
func (m *PeerStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.LastEndpointChange != nil {
		if vtmsg, ok := interface{}(m.LastEndpointChange).(interface {
			...
			encoded, err := proto.Marshal(m.LastEndpointChange)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
	...
	if m.LastHandshakeTime != nil {
		if vtmsg, ok := interface{}(m.LastHandshakeTime).(interface {
			...
			encoded, err := proto.Marshal(m.LastHandshakeTime)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
	...
	if m.TransmitBytes != 0 {
		i = encodeVarint(dAtA, i, uint64(m.TransmitBytes))
		i--
		dAtA[i] = 0x28
	}
	if m.ReceiveBytes != 0 {
		i = encodeVarint(dAtA, i, uint64(m.ReceiveBytes))
		i--
		dAtA[i] = 0x20
	}
	if m.State != 0 {
...
func (m *PeerStatusSpec) SizeVT() (n int) {
	...
	if m.ReceiveBytes != 0 {
		n += 1 + sov(uint64(m.ReceiveBytes))
	}
	if m.TransmitBytes != 0 {
		n += 1 + sov(uint64(m.TransmitBytes))
	}
	if m.LastHandshakeTime != nil {
		if size, ok := interface{}(m.LastHandshakeTime).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.LastHandshakeTime)
		}
		n += 1 + l + sov(uint64(l))
	}
	if m.LastUsedEndpoint != nil {
	...
	if m.LastEndpointChange != nil {
		if size, ok := interface{}(m.LastEndpointChange).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.LastEndpointChange)
		}
		n += 1 + l + sov(uint64(l))
	}
	n += len(m.unknownFields)
...
func (m *PeerStatusSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			...
			if m.LastHandshakeTime == nil {
				m.LastHandshakeTime = &timestamppb.Timestamp{}
			}
			if unmarshal, ok := interface{}(m.LastHandshakeTime).(interface {
				UnmarshalVT([]byte) error
			}); ok {
				if err := unmarshal.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.LastHandshakeTime); err != nil {
					return err
				}
			}
			iNdEx = postIndex
		...
		case 8:
			...
			if m.LastEndpointChange == nil {
				m.LastEndpointChange = &timestamppb.Timestamp{}
			}
			if unmarshal, ok := interface{}(m.LastEndpointChange).(interface {
				UnmarshalVT([]byte) error
			}); ok {
				if err := unmarshal.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.LastEndpointChange); err != nil {
					return err
				}
			}
			iNdEx = postIndex
