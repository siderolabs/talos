...
func (m *TaskStatusSpec) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *TaskStatusSpec) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TaskStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.TaskState != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.TaskState))
		i--
		dAtA[i] = 0x10
	}
	if len(m.Id) > 0 {
...
func (m *TaskStatusSpec) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if m.TaskState != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.TaskState))
	}
	l = len(m.Result)
	if l > 0 {
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
...
func (m *TaskStatusSpec) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TaskStatusSpec: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TaskStatusSpec: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.TaskState |= enums.RuntimeTaskState(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
