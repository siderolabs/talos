...
func (m *VolumeStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.EnrolledEncryptionKeys) > 0 {
		for iNdEx := len(m.EnrolledEncryptionKeys) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.EnrolledEncryptionKeys[iNdEx])
			copy(dAtA[i:], m.EnrolledEncryptionKeys[iNdEx])
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.EnrolledEncryptionKeys[iNdEx])))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xe2
...
func (m *VolumeStatusSpec) SizeVT() (n int) {
	...
	if len(m.EnrolledEncryptionKeys) > 0 {
		for _, s := range m.EnrolledEncryptionKeys {
			l = len(s)
			n += 2 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
...
func (m *VolumeStatusSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 28:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field EnrolledEncryptionKeys", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.EnrolledEncryptionKeys = append(m.EnrolledEncryptionKeys, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
