...
func init() {
	...
	KubeletNodeIPConfigDoc.Fields[0].Name = "subnets"
	KubeletNodeIPConfigDoc.Fields[0].Type = "[]string"
	KubeletNodeIPConfigDoc.Fields[0].Note = ""
	KubeletNodeIPConfigDoc.Fields[0].Description = "The `subnets` field configures the networks to pick kubelet node IP from.\nFor dual stack configuration, there should be two subnets: one for IPv4, another for IPv6.\nIf not specified, kubelet configures node IP automatically."
	KubeletNodeIPConfigDoc.Fields[0].Comments[encoder.LineComment] = "The `subnets` field configures the networks to pick kubelet node IP from."
