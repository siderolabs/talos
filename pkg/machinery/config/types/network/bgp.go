...
// BGPKind is a BGP config document kind.
const BGPKind = "BGPConfig"

func init() {
	registry.Register(BGPKind, func(version string) config.Document {
		switch version {
		case "v1alpha1": //nolint:goconst
			return &BGPConfigV1Alpha1{}
		default:
			return nil
		}
	})
...
// Check interfaces.
var (
	_ config.NetworkBGPConfig = &BGPConfigV1Alpha1{}
	_ config.Validator        = &BGPConfigV1Alpha1{}
)

// BGPConfigV1Alpha1 configures a native BGP speaker on the host.
//
//	examples:
//	  - value: exampleBGPConfigV1Alpha1()
//	alias: BGPConfig
//	schemaRoot: true
//	schemaMeta: v1alpha1/BGPConfig
type BGPConfigV1Alpha1 struct {
	...
	//   description: |
	//     Names of the interfaces whose addresses are originated into BGP as host routes (/32, /128).
...
type BGPNeighborConfig struct {
	//   description: |
	//     Neighbor IP address for a numbered session. Mutually exclusive with `interface`.
	//   schema:
	//     type: string
	NeighborAddressConfig meta.Addr `yaml:"address,omitempty"`
	//   description: |
	//     Interface name for an unnumbered (IPv6 link-local) session. Mutually exclusive with `address`.
	NeighborInterfaceConfig string `yaml:"interface,omitempty"`
	//   description: |
...
// NewBGPConfigV1Alpha1 creates a new BGPConfig config document.
func NewBGPConfigV1Alpha1() *BGPConfigV1Alpha1 {
	return &BGPConfigV1Alpha1{
		Meta: meta.Meta{
			MetaKind:       BGPKind,
...
func (s *BGPConfigV1Alpha1) Clone() config.Document {
	return s.DeepCopy()
}

// BGPConfig implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) BGPConfig() {}

// LocalASN implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) LocalASN() uint32 {
	return s.BGPLocalASN
}

// RouterID implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) RouterID() netip.Addr {
	return s.BGPRouterID.Addr
}

// RouteSource implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) RouteSource() netip.Addr {
	return s.BGPRouteSource.Addr
}

// AdvertiseLinks implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) AdvertiseLinks() []string {
	return s.BGPAdvertise
}

// Multipath implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) Multipath() bool {
	return s.BGPMultipath
}

// MaxPaths implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) MaxPaths() uint8 {
	return s.BGPMaxPaths
}

// Neighbors implements config.NetworkBGPConfig interface.
func (s *BGPConfigV1Alpha1) Neighbors() []config.NetworkBGPNeighbor {
	return xslices.Map(s.BGPNeighborConfigs, func(n BGPNeighborConfig) config.NetworkBGPNeighbor { return n })
}

// Validate implements config.Validator interface.
func (s *BGPConfigV1Alpha1) Validate(validation.RuntimeMode, ...validation.Option) ([]string, error) {
	var (
		errs     error
		warnings []string
	)
	...
	if len(s.BGPNeighborConfigs) == 0 {
		warnings = append(warnings, "BGPConfig has no neighbors configured")
	}

	for i, n := range s.BGPNeighborConfigs {
		hasAddress := n.NeighborAddressConfig.IsValid()
		hasInterface := n.NeighborInterfaceConfig != ""

		if hasAddress == hasInterface {
			errs = errors.Join(errs, fmt.Errorf("neighbor[%d]: exactly one of address or interface must be set", i))
		}
	}
...
// Interface implements config.NetworkBGPNeighbor interface.
func (n BGPNeighborConfig) Interface() string {
	return n.NeighborInterfaceConfig
}
