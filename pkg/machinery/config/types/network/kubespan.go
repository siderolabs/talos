...
// Check interfaces.
var (
	_ config.KubeSpan                     = &KubeSpanConfigV1Alpha1{}
...
func (s *KubeSpanConfigV1Alpha1) Validate(validation.RuntimeMode, ...validation.Option) ([]string, error) {
	if s.ConfigMTU != nil && *s.ConfigMTU < constants.KubeSpanLinkMinimumMTU {
		return nil, fmt.Errorf("kubespan link MTU must be at least %d", constants.KubeSpanLinkMinimumMTU)
	}
...
func (s *KubeSpanConfigV1Alpha1) V1Alpha1ConflictValidate(v1alpha1Cfg *v1alpha1.Config) error {
	if v1alpha1Cfg.Machine() != nil && v1alpha1Cfg.Machine().Network() != nil {
		legacyKubespan := v1alpha1Cfg.Machine().Network().KubeSpan()

		if legacyKubespan != nil && legacyKubespan.Enabled() {
...
// Enabled implements config.KubeSpan interface.
func (s *KubeSpanConfigV1Alpha1) Enabled() bool {
	return pointer.SafeDeref(s.ConfigEnabled)
}

// AdvertiseKubernetesNetworks implements config.KubeSpan interface.
...
// Filters implements config.KubeSpan interface.
func (s *KubeSpanConfigV1Alpha1) Filters() config.KubeSpanFilters {
