...
type VolumeSelector struct {
	...
	//      name: match xfs volume on disk /dev/sda
...
func exampleVolumeSelector2() cel.Expression {
	return cel.MustExpression(cel.ParseBooleanExpression(`volume.name == "xfs" && volume.parent_dev_path == "/dev/sda"`, celenv.VolumeLocator()))
}
