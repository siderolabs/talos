...
func TestExternalVolumeConfigValidate(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "virtiofs with NFS spec",

			cfg: func(t *testing.T) *block.ExternalVolumeConfigV1Alpha1 {
				c := block.NewExternalVolumeConfigV1Alpha1()
				c.MetaName = constants.EphemeralPartitionLabel
				c.FilesystemType = blockres.FilesystemTypeVirtiofs
				c.MountSpec.MountVirtiofs = &block.VirtiofsMountSpec{VirtiofsTag: "Data"}
				c.MountSpec.MountNFS = &block.NFSMountSpec{NFSServer: "10.0.0.10", NFSPath: "/export", NFSVersion: blockres.NFSVersion4Point1}

				return c
			},

			expectedErrors: "NFS mount spec is not allowed with virtiofs filesystem type",
		},
		{
			name: "NFS with virtiofs spec",
			...
			expectedErrors: "virtiofs mount spec is not allowed with NFS filesystem type",
		},
		{
