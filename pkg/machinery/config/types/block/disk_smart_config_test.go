...
func TestDiskSMARTConfigInterval(t *testing.T) {
	t.Parallel()

	cfg := block.NewDiskSMARTConfigV1Alpha1()

	// absent field -> default interval
	assert.Equal(t, constants.DefaultDiskSMARTInterval, cfg.Interval())

	cfg.SMARTInterval = time.Hour
