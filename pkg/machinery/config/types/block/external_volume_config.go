...
func (s *ExternalVolumeConfigV1Alpha1) Validate(validation.RuntimeMode, ...validation.Option) ([]string, error) {
	...
	case block.FilesystemTypeVirtiofs:
		...
		if s.MountSpec.MountNFS != nil {
			validationErrors = errors.Join(validationErrors, errors.New("NFS mount spec is not allowed with virtiofs filesystem type"))
		}
	...
	case block.FilesystemTypeNFS:
		...
		if s.MountSpec.MountVirtiofs != nil {
			validationErrors = errors.Join(validationErrors, errors.New("virtiofs mount spec is not allowed with NFS filesystem type"))
		}
