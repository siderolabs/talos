// Code generated by "enumer -type=VolumeType,VolumePhase,FilesystemType,EncryptionKeyType,EncryptionProviderType,FSParameterType,LVType -linecomment -text"; DO NOT EDIT.
...
const _VolumeTypeName = "partitiondisktmpfsdirectorysymlinkoverlayexternalphysical volumevolume grouplogical volume"

var _VolumeTypeIndex = [...]uint8{0, 9, 13, 18, 27, 34, 41, 49, 64, 76, 90}

const _VolumeTypeLowerName = "partitiondisktmpfsdirectorysymlinkoverlayexternalphysical volumevolume grouplogical volume"
...
var _VolumeTypeNames = []string{
	...
	_VolumeTypeName[49:64],
	_VolumeTypeName[64:76],
	_VolumeTypeName[76:90],
}
...
func (i LVType) String() string {
	if i < 0 || i >= LVType(len(_LVTypeIndex)-1) {
		return fmt.Sprintf("LVType(%d)", i)
	}
	return _LVTypeName[_LVTypeIndex[i]:_LVTypeIndex[i+1]]
}
...
// Throws an error if the param is not part of the enum.
func LVTypeString(s string) (LVType, error) {
	if val, ok := _LVTypeNameToValueMap[s]; ok {
		return val, nil
	}

	if val, ok := _LVTypeNameToValueMap[strings.ToLower(s)]; ok {
		return val, nil
	}
	return 0, fmt.Errorf("%s does not belong to LVType values", s)
}

// LVTypeValues returns all values of the enum
func LVTypeValues() []LVType {
	return _LVTypeValues
}

// LVTypeStrings returns a slice of all String values of the enum
func LVTypeStrings() []string {
	strs := make([]string, len(_LVTypeNames))
	copy(strs, _LVTypeNames)
	return strs
}

// IsALVType returns "true" if the value is listed in the enum definition. "false" otherwise
func (i LVType) IsALVType() bool {
	for _, v := range _LVTypeValues {
		if i == v {
			return true
		}
	}
...
// MarshalText implements the encoding.TextMarshaler interface for LVType
func (i LVType) MarshalText() ([]byte, error) {
	return []byte(i.String()), nil
}

// UnmarshalText implements the encoding.TextUnmarshaler interface for LVType
func (i *LVType) UnmarshalText(text []byte) error {
	var err error
	*i, err = LVTypeString(string(text))
	return err
}
