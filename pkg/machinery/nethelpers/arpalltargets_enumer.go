// Code generated by "enumer -type=ARPAllTargets,ARPValidate,AddressFlag,AddressSortAlgorithm,ADSelect,ADLACPActive,AutoHostnameKind,BondMode,BondXmitHashPolicy,ClientIdentifier,ConntrackState,DefaultAction,DNSProtocol,Duplex,Family,LACPRate,LinkFlag,LinkType,MatchOperator,NATType,NfTablesChainHook,NfTablesChainPriority,NfTablesVerdict,OperationalState,Port,PrimaryReselect,Protocol,RouteFlag,RouteProtocol,RouteType,RoutingRuleAction,RoutingTable,Scope,Status,VLANProtocol,WOLMode -linecomment -text"; DO NOT EDIT.
...
const _NATTypeName = "masqueradesnatdnat"

var _NATTypeIndex = [...]uint8{0, 10, 14, 18}

const _NATTypeLowerName = "masqueradesnatdnat"

func (i NATType) String() string {
	if i < 0 || i >= NATType(len(_NATTypeIndex)-1) {
		return fmt.Sprintf("NATType(%d)", i)
	}
	return _NATTypeName[_NATTypeIndex[i]:_NATTypeIndex[i+1]]
}
...
func _NATTypeNoOp() {
	var x [1]struct{}
	_ = x[NATTypeMasquerade-(0)]
	_ = x[NATTypeSNAT-(1)]
	_ = x[NATTypeDNAT-(2)]
}

var _NATTypeValues = []NATType{NATTypeMasquerade, NATTypeSNAT, NATTypeDNAT}

var _NATTypeNameToValueMap = map[string]NATType{
	_NATTypeName[0:10]:       NATTypeMasquerade,
	_NATTypeLowerName[0:10]:  NATTypeMasquerade,
	_NATTypeName[10:14]:      NATTypeSNAT,
	_NATTypeLowerName[10:14]: NATTypeSNAT,
	_NATTypeName[14:18]:      NATTypeDNAT,
	_NATTypeLowerName[14:18]: NATTypeDNAT,
}

var _NATTypeNames = []string{
	_NATTypeName[0:10],
	_NATTypeName[10:14],
	_NATTypeName[14:18],
}

// NATTypeString retrieves an enum value from the enum constants string name.
// Throws an error if the param is not part of the enum.
func NATTypeString(s string) (NATType, error) {
	if val, ok := _NATTypeNameToValueMap[s]; ok {
		return val, nil
	}

	if val, ok := _NATTypeNameToValueMap[strings.ToLower(s)]; ok {
		return val, nil
	}
	return 0, fmt.Errorf("%s does not belong to NATType values", s)
}

// NATTypeValues returns all values of the enum
func NATTypeValues() []NATType {
	return _NATTypeValues
}

// NATTypeStrings returns a slice of all String values of the enum
func NATTypeStrings() []string {
	strs := make([]string, len(_NATTypeNames))
	copy(strs, _NATTypeNames)
	return strs
}

// IsANATType returns "true" if the value is listed in the enum definition. "false" otherwise
func (i NATType) IsANATType() bool {
	for _, v := range _NATTypeValues {
		if i == v {
			return true
		}
	}
...
// MarshalText implements the encoding.TextMarshaler interface for NATType
func (i NATType) MarshalText() ([]byte, error) {
	return []byte(i.String()), nil
}

// UnmarshalText implements the encoding.TextUnmarshaler interface for NATType
func (i *NATType) UnmarshalText(text []byte) error {
	var err error
	*i, err = NATTypeString(string(text))
	return err
}
