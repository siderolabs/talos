...
func TestLLDPTestFrame(t *testing.T) {
	for _, portID := range []string{"veth12345678", "vethabcdef01"} {
		t.Run(portID, func(t *testing.T) {
			frame, err := vm.LLDPTestFrame(portID)
			...
			assert.Equal(t, portID, string(lldp.PortID.ID))
			assert.Equal(t, layers.LLDPPortIDSubtypeIfaceName, lldp.PortID.Subtype)
...
func TestLLDPTestFrameInvalidPortID(t *testing.T) {
	_, err := vm.LLDPTestFrame(strings.Repeat("x", 512))
	require.Error(t, err)
}
