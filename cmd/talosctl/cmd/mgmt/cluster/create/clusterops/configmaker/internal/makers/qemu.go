...
// initBGP starts an embedded gobgp fabric peer on the bridge gateway. Node-side BGPConfig is supplied
...
// config (a unique loopback on lo + an unnumbered BGPConfig peering over the fabric interfaces) is baked
...
// BGP) and an unnumbered BGPConfig peering with the host fabric peer over each fabric interface
...
func (m *Qemu) closNodeConfig(loopback, vip netip.Addr, asn uint32, ifaces []string) (*container.Container, error) {
	...
	bgp := networkcfg.NewBGPConfigV1Alpha1()
	bgp.BGPLocalASN = asn
