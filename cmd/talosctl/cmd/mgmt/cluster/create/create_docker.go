...
func getDockerClusterRequest(
	...
	if err != nil {
		return clusterCreateRequestData{}, fmt.Errorf("error parsing Talos version %q: %w", cOps.talosVersion, err)
	}

	if cOps.talosVersion[0] != 'v' {
		return clusterCreateRequestData{}, fmt.Errorf("invalid talos version %q, talos version must start with a %q", cOps.talosVersion, "v")
	}

	nodeImage := fmt.Sprintf("%s:%s", images.DefaultTalosImageRepository, cOps.talosVersion)
	...
	clusterRequest.Image = nodeImage
