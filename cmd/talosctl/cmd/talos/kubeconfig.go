...
If merge flag is false and [local-path] is "-", config will be written to stdout.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// the kubeconfig comes from the node, so it is untrusted input: make sure it is a plain
		// kubeconfig as generated by Talos before writing it out to the local filesystem.
		config, err := taloskubeconfig.LoadAndValidate(data)
		if err != nil {
			return err
		}

		if kubeconfigFlags.merge {
			return mergeKubeconfig(config, localPath)
		}
...
func mergeKubeconfig(config *clientcmdapi.Config, localPath string) error {
