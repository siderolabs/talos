...
func getKubernetesClient(ctx context.Context, c *client.Client) (*k8s.Clientset, error) {
	...
	// the kubeconfig comes from the node, so it is untrusted input: make sure it is
	// a plain kubeconfig as generated by Talos before handing it over to client-go.
	if _, err = kubeconfig.LoadAndValidate(kubeconfigBytes); err != nil {
		return nil, err
	}
