...
func (i *Installer) Install(ctx context.Context, mode Mode) (err error) {
	...
	case ModeInstall:
		if !i.options.Zero {
			...
			case info.Name == "":
				// empty, ok
			case info.Name == "gpt" && len(info.Parts) == 0:
	...
	if mode == ModeImage || mode == ModeInstall {
		...
		if err = i.createPartitions(gptdev, mode, hostTalosVersion, bootlder); err != nil {
	...
	if mode == ModeUpgrade || len(i.options.MetaValues.values) > 0 {
		...
		if mode == ModeUpgrade {
			if ok, err = metaState.SetTag(context.Background(), meta.Upgrade, bootInstallResult.PreviousLabel); !ok || err != nil {
...
//nolint:gocyclo,cyclop
func (i *Installer) createPartitions(gptdev gpt.Device, mode Mode, hostTalosVersion *compatibility.TalosVersion, bootlder bootloader.Bootloader) error {
	...
	for _, p := range partitions {
		...
		_, _, err = pt.AllocatePartition(size, p.PartitionLabel, partitionTyp)
		if err != nil {
			return fmt.Errorf("failed to allocate partition %s: %w", p.PartitionLabel, err)
		}

		i.options.Printf("created %s (%s) size %d blocks", p.PartitionLabel, p.PartitionType, size)
	}
