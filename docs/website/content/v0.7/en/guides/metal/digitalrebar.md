...
- 3 nodes (please see [hardware requirements](https://www.talos.dev/docs/v0.7/en/guides/getting-started#system-requirements))
- Loadbalancer
- Digital Rebar Server
- Talosctl access (see [talosctl setup](https://www.talos.dev/docs/v0.7/en/guides/getting-started/talosctl))
...
```bash
$ drpcli isos upload boot.tar.gz as talos-0.7.tar.gz
{
  "Path": "talos-0.7.tar.gz",
  "Size": 96470072
}
```
...
```bash
$ drpcli bootenvs create talos-0.7
{
  "Available": true,
  "BootParams": "",
  "Bundle": "",
...
```bash
drpcli bootenvs update talos-0.7 - < bootenv.yaml
```
...
To understand the boot process, we have a higher level overview located at [metal overview.](https://www.talos.dev/docs/v0.7/en/guides/metal/overview)
