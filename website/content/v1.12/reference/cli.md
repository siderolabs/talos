...
Note: exactly one of 'iso', 'pxe' or 'disk-image' presets must be specified.
...
```
  ...
  -h, --help                                     help for qemu
      ...
      --memory-controlplanes string(mb,gb)       the limit on memory usage for each control plane/VM (default 2.0GiB)
      --memory-workers string(mb,gb)             the limit on memory usage for each worker/VM (default 2.0GiB)
      --omni-api-endpoint string                 the omni api end point (must include a scheme, a port and a join token)
...
```
  ...
  -i, --image string               the container image to use for performing the install (default "ghcr.io/siderolabs/installer:v1.12.0-alpha.1")
