...
```
      ...
      --cni-bundle-url string                    URL to download CNI bundle from (default "https://github.com/siderolabs/talos/releases/download/v1.15.0-alpha.0/talosctl-cni-bundle-${ARCH}.tar.gz")
      ...
      --disk-driver string                       driver for the primary (system) disks (virtio, ide, ahci, scsi, nvme, megaraid, usb, mmc) (default "virtio")
      ...
      --extra-disks-drivers strings              driver for each extra disk (virtio, ide, ahci, scsi, nvme, megaraid, usb, mmc, virtiofs)
...
```
      ...
      --cpus-controlplanes string                the share of CPUs as fraction for each control plane/VM (default "2.0")
      --cpus-workers string                      the share of CPUs as fraction for each worker/VM (default "2.0")
      --disks disks                              list of disks to create in format "<driver1>:<size1>", drivers: virtio, ide, ahci, scsi, nvme, megaraid, usb, mmc, virtiofs (disks after the first one are added only to worker machines) (default virtio:10GiB,virtio:6GiB)
...
## talosctl recovery-key fetch

Fetch the recovery keys generated by the nodes
...
```
talosctl recovery-key fetch [flags]
```
...
```
  ...
  -e, --endpoints strings          override default endpoints in Talos configuration
  -i, --insecure                   use the insecure (encrypted with no auth) maintenance service
      --key-file string            file to read the recovery key from ('-' for stdin); prompted for on the terminal if not set
  -n, --nodes strings              target the specified nodes
      --siderov1-keys-dir string   the path to the SideroV1 auth PGP keys directory, defaults to 'SIDEROV1_KEYS_DIR' env variable if set, otherwise '$HOME/.talos/keys'; only valid for Contexts that use SideroV1 auth
      --talosconfig string         the path to the Talos configuration file, defaults to 'TALOSCONFIG' env variable if set, otherwise '$HOME/.talos/config' and '/var/run/secrets/talos.dev/config' in order
      --timeout duration           time to wait for the volumes to be unlocked (default 1m0s)
      --volumes strings            volume IDs to operate on (default: all applicable volumes)
```

### SEE ALSO

* [talosctl recovery-key](#talosctl-recovery-key)	 - Manage disk encryption recovery keys

## talosctl recovery-key unlock

Unlock the locked volumes with the recovery key
...
```
talosctl recovery-key unlock [flags]
```
...
## talosctl recovery-key verify

Verify the recovery key against the volumes

### Synopsis

Verify the recovery key against the recovery key slot of the volumes.

Nothing is changed on the node. The command fails if the key doesn't match any of the volumes.

```
talosctl recovery-key verify [flags]
```
...
## talosctl recovery-key

Manage disk encryption recovery keys
...
* [talosctl](#talosctl)	 - A CLI for out-of-band management of Kubernetes nodes created by Talos
* [talosctl recovery-key fetch](#talosctl-recovery-key-fetch)	 - Fetch the recovery keys generated by the nodes
* [talosctl recovery-key unlock](#talosctl-recovery-key-unlock)	 - Unlock the locked volumes with the recovery key
* [talosctl recovery-key verify](#talosctl-recovery-key-verify)	 - Verify the recovery key against the volumes
...
```
  ...
  -i, --image string               the container image to use for performing the install (default "factory.talos.dev/metal-installer/376567988ad370138ad8b2698212367b8edcb69b5fd68c80be1f2ec7d603b4ba:v1.15.0-alpha.0")
