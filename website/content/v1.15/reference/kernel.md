...
The GPT partition UUID of the partition Talos was booted from.

The GRUB configuration generated by Talos sets this argument automatically (GRUB probes the partition it was loaded from,
the `BOOT` partition of the install disk), so it should not be set manually.
On UEFI systems booted via `systemd-boot` the same information comes from the `LoaderDevicePartUUID` EFI variable, and the argument is not needed.
On kexec (the bootloader is skipped) Talos passes this argument to the new kernel, as the EFI variable still describes the previous boot;
if set, the argument takes precedence over the EFI variable.

Talos uses the boot partition to wait for the disk it was booted from to be discovered before declaring
the system volumes (`META`, `STATE`) missing, as some storage controllers enumerate their disks well after `udevd` settles.
The detected boot partition is published as the `bootpartition` resource (`talosctl get bootpartition`).

Example:

```text
talos.boot.partuuid=6f5a6e8a-9c79-4c0f-8f35-0c2a1f1a0001
```
