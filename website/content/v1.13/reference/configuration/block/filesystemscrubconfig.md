...
mountpoint: /var
{{< /highlight >}}</details> | |
|`period` |Duration |Period for running the scrub task for this filesystem.<br><br>The first run is scheduled randomly within this period from the boot time, later ones follow after the full period.<br><br>Default value is 1 week, minimum value is 10 seconds.  | |
