...
|`strictCgroupClassOrdering` |bool |Whether to enforce strict QoS class ordering when selecting an OOM victim.<br><br>When enabled (the default), cgroups in the lowest-importance QoS class are<br>killed first, and the ranking score only breaks ties within a class.<br>When disabled, the highest-scoring cgroup is killed regardless of class.  | |
