...
5. **Import snapshot from object storage** (repeat for each zone you need):
  ```bash
...
6. **Create image from snapshot** (optional - instances can be created directly from snapshots):
   ```bash
...
7. **Create instance** using the snapshot/image via GUI, CLI, or Infrastructure as Code tools.
