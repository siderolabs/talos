...
```
  ...
  -p, --exposed-ports string                     (docker only) Comma-separated list of ports/protocols to expose on init node. Ex -p <hostPort>:<containerPort>/<protocol (tcp or udp)>
      ...
      --mount mount                              (docker only) attach a mount to the container (Docker only)
...
Creates a local qemu based kubernetes cluster
...
* [talosctl cluster create qemu](#talosctl-cluster-create-qemu)	 - Creates a local qemu based kubernetes cluster
