...
description: |
    ...
    The speaker originates the addresses of the listed interfaces as host routes and installs the
    routes it learns from its neighbors into the kernel routing table.
...
{{< /highlight >}}</details> | |
|`advertise` |[]string |Names of the interfaces whose addresses are originated into BGP as host routes (/32, /128).<br>Typically a loopback or dummy interface holding the node IP. <details><summary>Show example(s)</summary>{{< highlight yaml >}}
advertise:
    - dummy0
{{< /highlight >}}</details> | |
...
|`neighbors` |<a href="#BGPConfigV1Alpha1.neighbors.">[]BGPNeighborConfig</a> |BGP neighbors to peer with.  | |




## neighbors[] {#BGPConfigV1Alpha1.neighbors.}
...
| Field | Type | Description | Value(s) |
|-------|------|-------------|----------|
|`address` |Addr |Neighbor IP address for a numbered session. Mutually exclusive with `interface`.  | |
|`interface` |string |Interface name for an unnumbered (IPv6 link-local) session. Mutually exclusive with `address`.  | |
...
|`bfd` |<a href="#BGPConfigV1Alpha1.neighbors..bfd">BGPBFDConfig</a> |BFD (Bidirectional Forwarding Detection) configuration for the neighbor.  | |




### bfd {#BGPConfigV1Alpha1.neighbors..bfd}
