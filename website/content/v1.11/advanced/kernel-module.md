...
```
Create a pkg.yaml file which works similarly to our pkg.yaml file for our package, but this time starts from the base image we built in the first step.
...
```bash
make target-my-module REGISTRY=127.0.0.1:5005 \
  PLATFORM=linux/amd64 \
  PUSH=true
```
...
Save the image as `${EXTENSION_IMAGE}`.
...
Build the installer, and remember to use the kernel image from the previous step.
