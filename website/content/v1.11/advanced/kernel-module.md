...
```bash
name: my-module   # name of your package
...
steps:            # steps needed to build package container
  - sources:      # download source files
    ...
    prepare:      # create directories and untar
      - tar -xzf my-medule.tar.gz --strip-components=1
      - mkdir build
      - cd build
...
```bash
make kernel my-module REGISTRY=127.0.0.1:5005 PUSH=true PLATFORM=linux/amd64
```
...
...
=> => pushing manifest for 127.0.0.1:5005/user/my-module:v1.11.0-alpha.0...
```
...
The only unique thing about build this system extension is we need to build it against the kernel we just built in the previous step.
...
Now create the `manifest.yaml` file for the metadata of your extension in the my-extension folder.
```yaml
version: v1alpha1       # version of manifest.yaml
metadata:
  name: my-extension    # name of extension
  version: 0.1
  author: me
  description: |
...
```
Create a pkg.yaml file which works similarly to our previous pkg.yaml file, but this time starts from the base image we built in the first step.
...
finalize:
  - from: /rootfs
    to: /rootfs
  - from: /pkg/manifest.yaml  # make sure you add the metadata file to the root
    to: /
```
...
```bash
make my-extension REGISTRY=127.0.0.1:5005 PUSH=true PLATFORM=linux/amd64 \
    PKG_KERNEL=${KERNEL_IMAGE}
```
This will create a system extension image and push it to your local registry.
Write down the image that get's pushed for the next step.

## Test the extension

Now we need to create installation media to boot Talos.
We will use [imager]({{< relref "../talos-guides/install/boot-assets#imager" >}}) with a configuration file to include our extension.

Go back into the pkg repo and build the installer.
Remember to use the kernel image from the previous step.

```bash
cd pkg
make installer PLATFORM=linux/amd64 INSTALLER_ARCH=amd64 PUSH=true REGISTRY=127.0.0.1:5005 \
  PKG_KERNEL=${KERNEL_IMAGE}
```
This will create an imager image and push it to your local registry.
Write down that image and save it as `$IMAGER_IMAGE`.

Create a profile.yaml file with the configuration for what asset we want to build.
...
platform: metal
secureboot: false
version: v1.10.0                      # update the version of Talos here
input:
  kernel:
  ...
  baseInstaller:
    imageRef: ghcr.io/siderolabs/installer:v1.10.0  # update talos installer version
  systemExtensions:
    - imageRef: "$EXTENSION_IMAGE"    # Use the image created in the 2nd step
output:
  kind: installer
  outFormat: raw
```
This will create a installer container image which we'll reference in our Talos machine configuration.

First we need to build the installer.
```bash
cat profile.yaml | docker run --rm -i -v $PWD/_out:/out ${IMAGER_IMAGE} -
```
We'll have a new container image tar file in the _out/ folder of our repository.
Load the image into docker and push it to a registry that your machine has access to.

```bash
docker load -i ./_out/installer-amd64.tar
docker tag ghcr.io/siderolabs/installer:v1.10.0 my-installer:v1.10.0
docker push my-installer:v1.10.0
```
...
```bash
talosctl gen config --install-image my-installer:v1.10.0 \
