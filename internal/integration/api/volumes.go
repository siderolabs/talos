...
// TestExternalVolumesNFS performs a series of operations on external NFS volumes: mount/unmount, etc.
func (suite *VolumesSuite) TestExternalVolumesNFS() {
	...
	externalVolumeDoc.FilesystemType = block.FilesystemTypeNFS
	externalVolumeDoc.MountSpec = blockcfg.ExternalMountSpec{
		MountSource: fmt.Sprintf("%s:/", suite.Cluster.Info().Network.GatewayAddrs[0].String()),
		MountNFS: &blockcfg.NFSMountSpec{
			NFSVersion:  block.NFSVersionType4,
