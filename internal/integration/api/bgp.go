...
func (suite *BGPSuite) TestNumberedBGP() {
	...
	bgp := network.NewBGPConfigV1Alpha1()
	...
	suite.RemoveMachineConfigDocuments(nodeCtx, network.BGPKind)
...
// only via BGP. The per-node config (loopback on lo + unnumbered BGPConfig over the fabric uplinks) is
