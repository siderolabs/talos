...
func (suite *ExtensionsSuiteNVIDIA) TestExtensionsNVIDIA() {
	...
	suite.Require().NoError(retry.Constant(4*time.Minute, retry.WithUnits(5*time.Second)).Retry(
		func() error {
			...
			if toolkitDaemonSet.Status.DesiredNumberScheduled == 0 ||
				toolkitDaemonSet.Status.NumberReady != toolkitDaemonSet.Status.DesiredNumberScheduled {
	...
	suite.Require().NoError(retry.Constant(4*time.Minute, retry.WithUnits(5*time.Second)).Retry(
		func() error {
			...
			if devicePluginDaemonSet.Status.DesiredNumberScheduled == 0 ||
				devicePluginDaemonSet.Status.NumberReady != devicePluginDaemonSet.Status.DesiredNumberScheduled {
				return retry.ExpectedErrorf(
