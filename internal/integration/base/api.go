...
func (apiSuite *APISuite) DiscoverNodeInternalIPs(ctx context.Context) []string {
	nodes, err := apiSuite.DiscoverNodes(ctx).Nodes()
	apiSuite.Require().NoError(err)
...
func (apiSuite *APISuite) DiscoverNodeInternalIPsByType(ctx context.Context, machineType machine.Type) []string {
	nodesByType, err := apiSuite.DiscoverNodes(ctx).NodesByType(machineType)
	apiSuite.Require().NoError(err)
...
func (apiSuite *APISuite) RandomDiscoveredNodeInternalIP(types ...machine.Type) string {
	...
	if len(types) == 0 {
		...
		for _, t := range types {
			nodeInfosByType, err := nodeInfo.NodesByType(t)
			apiSuite.Require().NoError(err)
...
func (apiSuite *APISuite) ClearConnectionRefused(ctx context.Context, nodes ...string) {
	...
	controlPlaneNodes, err := apiSuite.DiscoverNodes(ctx).NodesByType(machine.TypeControlPlane)
	apiSuite.Require().NoError(err)

	initNodes, err := apiSuite.DiscoverNodes(ctx).NodesByType(machine.TypeInit)
	apiSuite.Require().NoError(err)
