...
var (
	...
	notFoundRE = regexp.MustCompile(`(No such file or directory|does not appear to be an md device|cannot find|No array with redundancy detected)`)
	...
	// mdadm emits a mix of usage/help text and explicit "invalid" messages when
	// the command line is rejected.
	invalidArgumentRE = regexp.MustCompile(`(?i)(invalid (option|argument)|unrecognized option|usage:|need at least|must be specified)`)

	// resyncRE matches reshape/grow attempts rejected while the array is still
	// synchronizing newly added members.
	resyncRE = regexp.MustCompile(`(performing resync/recovery and cannot be reshaped|resync or recovery in progress)`)
)
...
func sentinelFor(exit *cmd.ExitError) error {
	...
	case notFoundRE.Match(exit.Output):
		return ErrNotFound
	case invalidArgumentRE.Match(exit.Output):
		return ErrInvalidArgument
	case resyncRE.Match(exit.Output):
		return ErrResync
	}
