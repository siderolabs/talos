...
func SystemMountPointsForDevice(devpath string) (mountpoints *Points, err error) {
	...
	for _, name := range []string{universe.EphemeralPartitionLabel, universe.BootPartitionLabel} {
		var target string

		switch name {
		case universe.EphemeralPartitionLabel:
			target = universe.EphemeralMountPoint
		case universe.BootPartitionLabel:
			target = universe.BootMountPoint
		}

		var dev *probe.ProbedBlockDevice

		if dev, err = probe.DevForFileSystemLabel(devpath, name); err != nil {
			if name == universe.BootPartitionLabel {
...
func SystemMountPointForLabel(label string, opts ...Option) (mountpoint *Point, err error) {
	...
	if dev, err = probe.GetDevWithFileSystemLabel(label); err != nil {
		...
		if label == universe.BootPartitionLabel {
			return nil, nil
		}
