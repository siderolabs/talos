...
func (p *Point) Mount(opts Options) error {
	...
	return p.retry(func() error {
		if err := p.moveMount(p.target, 0); err != nil {
...
func (p *Point) Move(newTarget string) error {
	return p.moveMount(newTarget, unix.O_CLOEXEC)
}
...
func (p *Point) moveMount(target string, flags int) error {
	fd, err := p.root.Fd()
	if err != nil {
		if p.Source() != "" {
			if err := unix.MoveMount(unix.AT_FDCWD, p.Source(), unix.AT_FDCWD, target, flags); err != nil {
	...
	if err := unix.MoveMount(fd, "", unix.AT_FDCWD, target, flags); err != nil {
...
func (p *Point) setattr(attr *unix.MountAttr) error {
	fd, err := p.root.Fd()
	if err != nil {
		return err
	}
