...
// WithRepairOnUnmount triggers fs repair on unmount.
func WithRepairOnUnmount() ManagerOption {
	return ManagerOption{
		set: func(m *Manager) {
			m.extraUnmountCallbacks = append(m.extraUnmountCallbacks, func(m *Manager) {
				if err := m.fs.Repair(); err != nil && !errors.Is(err, fsopen.ErrRepairUnsupported) {
					panic(err)
				}
			})
		},
