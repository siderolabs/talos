...
func TestSessionDeadlineClosesHandshakeReadStall(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		go func() {
			_, err := libvirtstorage.New("", "storage:///system").OpenConn(ctx, raw, cancel)
			done <- err
		}()
	...
	for _, cancelCaller := range []bool{false, true} {
		t.Run(fmt.Sprintf("cancel=%t", cancelCaller), func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				client, err := libvirtstorage.New("", "storage:///system").OpenConn(sessionCtx, raw, cancelSession)
				require.NoError(t, err)
...
func TestSessionGracefulClose(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		client, err := libvirtstorage.New("", "storage:///system").OpenConn(ctx, raw, cancel)
		require.NoError(t, err)
		client.Close()
