...
func Seal(key []byte, tpmPCRs []int) (*SealedResponse, error) {
	...
	sealingPolicyDigest, err := calculateSealingPolicyDigest(t, tpmPCRs)
	if err != nil {
...
func calculateSealingPolicyDigest(t transport.TPM, pcrs []int) ([]byte, error) {
	if len(pcrs) > 0 {
