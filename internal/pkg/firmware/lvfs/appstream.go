...
// CabURL returns the download location of the release container, preferring
// absolute artifact locations over the (possibly relative) release location.
func (r *Release) CabURL() string {
...
// CabSHA256 returns the SHA-256 checksum of the release container, if present.
func (r *Release) CabSHA256() string {
	for _, artifact := range r.Artifacts {
		if artifact.Type != "binary" {
			continue
		}
		...
		for _, checksum := range artifact.Checksums {
			if checksum.Type == "sha256" {
				return strings.TrimSpace(checksum.Value)
			}
		}
	}
