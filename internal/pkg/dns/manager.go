...
func (m *Manager) RunAll(pairs iter.Seq[AddressPair], forwardEnabled bool) iter.Seq2[RunResult, error] {
	return func(yield func(RunResult, error) bool) {
		...
		for cfg := range pairs {
			...
			// The runner rebuilds its server and listeners on every (re)start,
			// so opts (which bind sockets) are constructed lazily per start
			// rather than once up front. A persistent construction failure is
			// reported through the supervisor's event hook and retried.
			newOpts := func() (RunnerOptions, error) {
				opts, err := newDNSRunnerOpts(cfg, m.cacheHandler, forwardEnabled)
				if err != nil {
					return RunnerOptions{}, fmt.Errorf("%w: %w", ErrCreatingRunner, err)
				}
			...
			if !yield(makeResult(cfg, StatusNew), nil) {
				return
			}
		}
