...
func buildContainerRows(
	...
	for name, status := range statuses {
		...
		// The Stats API is keyed by the containerd ID, which is the instance rather than the
		// container: every restart runs under a new ID.
		statsID := containers.InstanceID(name, spec.RestartCount)
		...
		row := containerRow{
			...
			PID:        spec.PID,
			CPUPercent: cpuPercent(cpuDiff, statsID, interval),
			Image:      spec.Image,
			Error:      spec.Error,
		}

		if stat := stats[statsID]; stat != nil {
...
// containerLogsAvailable reports whether the container has a log to stream.
//
// The runner registers the log only when it starts a task, so a container that has never gotten
// past creating one has nothing to read, and asking for it fails with "log was not registered".
// The log outlives its instance, so an exited or backing-off container still has one.
func containerLogsAvailable(instances []*containers.ContainerInstanceStatus) bool {
	return slices.ContainsFunc(instances, func(instance *containers.ContainerInstanceStatus) bool {
		switch instance.TypedSpec().Phase {
		case containers.ContainerInstancePhaseRunning,
			containers.ContainerInstancePhaseTerminated,
			containers.ContainerInstancePhaseFailed:
			return true
...
func formatMounts(mounts []containers.ContainerMountSpec) string {
	...
	for _, mount := range mounts {
		...
		// Parenthesized rather than bracketed: square brackets are tview's color-tag syntax, and
