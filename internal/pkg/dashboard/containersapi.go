...
func (widget *ContainersGrid) startAPIPoll() {
	...
	// The node is captured here, on the UI goroutine: OnNodeSelect writes it, and a poll that has
	// been cancelled but not yet returned must not read it.
	go widget.runAPIPoll(ctx, widget.selectedNode, containerNamespaces[widget.nsIndex])
}
...
func (widget *ContainersGrid) runAPIPoll(ctx context.Context, node string, target containerNamespace) {
