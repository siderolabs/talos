...
//
// The filter goes with them: it was typed against the stream being left, and carrying it over would
// silently hide the new stream's lines with the filter input itself hidden. The focus is left alone,
// because the callers move it themselves once they have switched the view over.
func (widget *LogViewer) Reset() {
	widget.entries = nil

	// Clearing the input fires the changed func, which renders the (now empty) entries; the
	// placeholder below is written after it, not before.
	widget.filterText = ""
