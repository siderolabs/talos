...
func TestHeaderEscapesAPIData(t *testing.T) {
	...
	t.Run("hostname", func(t *testing.T) {
		...
		widget.OnNodeSelect(node)
		widget.OnResourceDataChange(resourcedata.Data{Node: node, Resource: hostname})
	...
	t.Run("version", func(t *testing.T) {
		...
		widget := components.NewHeader()
		widget.OnNodeSelect(node)
		widget.OnAPIDataChange(node, &apidata.Data{
			Nodes: map[string]*apidata.Node{
				node: {
					Version: &machine.Version{
						Version: &machine.VersionInfo{Tag: payload},
					},
				},
			},
		})
