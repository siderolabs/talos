...
const (
	...
	// watchRetryInterval is the wait before a watch that failed is started over. It matches the log
	// stream's retry: the failure is usually a node that went away, and those come back on that
	// scale.
	watchRetryInterval = 5 * time.Second
)

// The dashboard discovers what a screen listens to by asserting these interfaces on it, which is a
// wiring that breaks silently: a screen that stops satisfying one simply stops being updated.
var (
	_ APIDataListener    = (*ContainersGrid)(nil)
	_ NodeSelectListener = (*ContainersGrid)(nil)
	_ TickerListener     = (*ContainersGrid)(nil)
)
...
type ContainersGrid struct {
	...
	// generation invalidates the results of sources that have already been stopped: canceling a
	// watch or a poll does not retract the updates it has already queued, and applying one would
	// paint the previous node's or namespace's containers over the current ones. Every source
	// carries the generation it was started with, and only writes when it still matches.
	generation uint64

	// pendingRefresh records that an update arrived while the dashboard was paused, so that the
	// next tick after it resumes renders what was skipped.
	pendingRefresh bool
...
func (widget *ContainersGrid) OnTick() {
	if !widget.active {
		return
	}

	// The field summary renders an uptime, which is derived from the clock rather than from an
	// event: on a quiet node it would otherwise stop counting. The instance history is all absolute
	// timestamps, so it has nothing to gain from a tick.
	if widget.pendingRefresh || (widget.level == 1 && !widget.showInstances) {
		widget.refresh()
	}
}
...
func (widget *ContainersGrid) handleKey(event *tcell.EventKey) *tcell.EventKey {
	...
	case 's':
		// The API-mode list is always ordered by ID, so the sort has nothing to apply to there and
		// the list title does not offer it.
		if widget.level == 0 && widget.talosMode() {
...
func (widget *ContainersGrid) startWatch() {
	...
	// node, and a watch that has been canceled but not yet returned must neither read it nor write
	// anything back.
	go widget.runWatch(ctx, widget.selectedNode, widget.generation)
}
...
func (widget *ContainersGrid) runWatch(ctx context.Context, node string, generation uint64) {
	eventCh := make(chan state.Event)

	if !widget.establishWatches(utils.NodeContext(ctx, node), eventCh, generation) {
		return
	}
	...
	for {
		...
		case <-ctx.Done():
			return
		case event := <-eventCh:
			widget.handleWatchEvent(event, generation)

			if event.Type == state.Errored {
				widget.restartWatch(ctx, generation)
...
// establishWatches opens the watch of every kind the screen tracks, and reports whether they are
// all running: a failure to open one is shown in place of the list, since the join it feeds would
// be missing a side.
func (widget *ContainersGrid) establishWatches(nodeCtx context.Context, eventCh chan state.Event, generation uint64) bool {
...
// restartWatch waits a failure out and starts the watch over.
//
// COSI delivers nothing further on a watch that errored, so the kinds it carried would quietly stop
// updating while the rest stayed live. Starting over also re-bootstraps the contents the screen
// missed in the meantime.
func (widget *ContainersGrid) restartWatch(ctx context.Context, generation uint64) {
	select {
	case <-ctx.Done():
		return
...
func (widget *ContainersGrid) handleWatchEvent(event state.Event, generation uint64) {
	widget.app.QueueUpdate(func() {
		if widget.stale(generation) {
			return
		}
...
// stale reports whether the result of a source belongs to a generation that has been superseded,
// i.e. whether the source it came from has been stopped in the meantime.
func (widget *ContainersGrid) stale(generation uint64) bool {
	return generation != widget.generation
}

// refresh re-renders whatever the screen currently shows, from the state the sources have gathered.
//
// It is what every live update goes through, as opposed to a navigation: the panes keep their scroll
// offsets and their cursors, and nothing is rendered at all while the dashboard is paused.
func (widget *ContainersGrid) refresh() {
	if widget.dashboard.paused {
		widget.pendingRefresh = true
	...
	// In API mode the list is rebuilt even when it is not the visible level: the detail pane renders
	// a row taken from it, which would otherwise stay frozen at the sample the operator drilled into.
	if widget.level == 0 || !widget.talosMode() {
		widget.renderList()
	}
	...
	if !widget.talosMode() {
		widget.refreshSelectedAPIRow()
	}

	widget.renderDetail(false)

	// The YAML pane is only reachable in Talos mode, and it renders resources that keep changing
	// underneath it: leaving it alone would freeze it at whatever the drill-down happened to catch.
	if widget.level == 2 && widget.selected != "" {
		widget.renderYAML(widget.selected, widget.yamlKind)
	}
}
...
// resetLogs clears the log pane along with the scrollback the source has already delivered, so that
// the next stream for the same container tails again rather than opening onto an empty pane.
func (widget *ContainersGrid) resetLogs() {
	widget.logViewer.Reset()
	widget.logSource.Forget()
}
...
// renderYAML fills the YAML pane with one of the container's resources.
//
// It is what a live update goes through: the navigation state and the scroll offset are left alone,
// so a resource that appears while the pane is open replaces the "not available" it was showing
// without moving the pane underneath the operator.
func (widget *ContainersGrid) renderYAML(name string, kind int) {
