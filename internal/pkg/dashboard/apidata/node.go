...
func (node *Node) UpdateDiff(old *Node) {
	...
	if old.ContainerStats != nil {
		...
		stats := node.ContainerStats.GetStats()

		node.ContainerCPUDiff = make(map[string]uint64, len(stats))

		for _, stat := range stats {
			if diff, ok := statCPUDiff(index[stat.Id], stat); ok {
				node.ContainerCPUDiff[stat.Id] = diff
			}
		}
	}
}
