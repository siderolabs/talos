...
func run() error {
	...
	pseudo := mount.Pseudo(log.Printf)

	if _, err := pseudo.Mount(); err != nil {
		return err
	}

	if _, err := mount.PseudoSub(log.Printf).Mount(); err != nil {
		return err
	}
...
func mountRootFS() error {
	...
	if len(extensionsConfig.Layers) == 0 {
		...
		if err != nil {
			return fmt.Errorf("failed to initialize sqashfs: %w", err)
		}

		if _, err := squashfs.Mount(); err != nil {
			return fmt.Errorf("failed to mount sqashfs: %w", err)
		}
	...
	for _, layer := range layers {
		...
		point, err := mount.Squashfs(target, layer.image, log.Printf)
		if err != nil {
			return err
		}

		squashfs = append(squashfs, point)
