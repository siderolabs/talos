...
func (g *grpcStdioStreamer) streamHostNs(ctx context.Context, exitC <-chan int, controlC chan<- hostNsControl) error {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	...
	go func(errCh chan<- error) {
		errCh <- panicsafe.RunErr(func() error {
			return g.recvLoopHostNs(ctx, controlC)
		})
	}(recvLoopCh)
...
func (g *grpcStdioStreamer) recvLoopHostNs(ctx context.Context, controlC chan<- hostNsControl) error {
	defer g.stdinW.Close() //nolint:errcheck
