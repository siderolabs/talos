	...
	if err := safe.WriterModify(ctx, runtime,
		...
		func(res *containers.ContainerInstanceStatus) error {
			*res.TypedSpec() = previousStatusSpec
			res.Metadata().Labels().Set(containers.ContainerSpecIdLabel, previousStatusSpec.ContainerID)
...
func (ctrl *RuntimeController) writeStatus(
	...
	if err := safe.WriterModify(ctx, runtime,
		...
		func(res *containers.ContainerInstanceStatus) error {
			instanceStatus := res.TypedSpec()

			instanceStatus.ContainerID = containerInstanceSpec.TypedSpec().ContainerID
			instanceStatus.Generation = containerInstanceSpec.TypedSpec().Generation
			instanceStatus.Phase = snapshot.Phase
			instanceStatus.PID = snapshot.PID
			instanceStatus.ExitCode = snapshot.ExitCode
			instanceStatus.Error = snapshot.Error
			instanceStatus.StartedAt = snapshot.StartedAt
			instanceStatus.FinishedAt = snapshot.FinishedAt

			res.Metadata().Labels().Set(containers.ContainerSpecIdLabel, containerInstanceSpec.TypedSpec().ContainerID)
