...
func (ctrl *LoadedKernelModuleController) reconcile(ctx context.Context, r controller.Runtime, path string) error {
	...
	touchedIDs := make(map[string]struct{}, len(rawModules))
	...
	for _, module := range rawModules {
		...
		// mark the module as touched
		touchedIDs[module.Name] = struct{}{}
	}

	// remove kernel modules that were not touched
	modules, err := safe.ReaderListAll[*runtime.LoadedKernelModule](ctx, r)
	if err != nil {
		return fmt.Errorf("failed to list loaded kernel modules: %w", err)
	}

	for module := range modules.All() {
		if _, ok := touchedIDs[module.Metadata().ID()]; ok {
			continue
		}

		if err = r.Destroy(ctx, module.Metadata()); err != nil && !state.IsNotFoundError(err) {
			return fmt.Errorf("failed to remove loaded kernel module: %w", err)
		}
	}
