...
func TestCRIImageGC(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		controller := &runtimectrl.CRIImageGCController{
		...
		storedImages := []images.Image{
			{
				...
				CreatedAt: now.Add(-2 * runtimectrl.ImageGCGracePeriod),
			...
			}, // ok to be gc'd
			{
				...
				CreatedAt: now.Add(-4 * runtimectrl.ImageGCGracePeriod),
				Target: v1.Descriptor{
					Digest: must(digest.Parse("sha256:6b094bd0b063a1172eec7da249eccbb48cc48333800569363d67c747960cfa0a")),
				},
			...
			}, // current image, digest ref
			{
				...
				CreatedAt: now.Add(runtimectrl.ImageGCGracePeriod),
				Target: v1.Descriptor{
		...
		time.Sleep(runtimectrl.ImageGCGracePeriod + 5*time.Minute)
		synctest.Wait()

		// Advance time to trigger the cleanup cycle (15 minutes)
		time.Sleep(runtimectrl.ImageCleanupInterval)
