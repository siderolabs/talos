...
func (ctrl *MountController) handleMountOperation(
	...
	case block.VolumeTypeBind:
		return ctrl.handleBindMountOperation(logger, rootPath, mountTarget, mountRequest, volumeStatus)
...
func (ctrl *MountController) handleBindMountOperation(
	...
	if !ok {
		// TODO:
		// - create source directory in /var/mnt/<meta-id>
		// - bind mount it over itslef as read/write
		mountTarget := filepath.Join(rootPath, target)
		...
		manager := mount.NewManager(slices.Concat(
			[]mount.ManagerOption{
				mount.WithTarget(mountTarget),
				mount.WithOpentree(mountTarget),
				mount.WithPrinter(logger.Sugar().Infof),
			},
			opts,
		)...)
...
func (ctrl *MountController) handleUnmountOperation(
	...
	case block.VolumeTypeDirectory:
		return nil
	case block.VolumeTypeBind:
		return ctrl.handleBindUnmountOperation(logger, mountRequest, volumeStatus)
	case block.VolumeTypeTmpfs:
		return fmt.Errorf("not implemented yet")
...
// TODO: @shanduur - fix this.
func (ctrl *MountController) handleBindUnmountOperation(
	logger *zap.Logger,
	mountRequest *block.MountRequest,
	_ *block.VolumeStatus,
) error {
	...
	if err := mountCtx.unmounter(); err != nil { // TODO: filter out error is not mounted
		return fmt.Errorf("failed to unmount %q: %w", mountRequest.Metadata().ID(), err)
	}

	if err := os.RemoveAll(mountCtx.point.Source()); err != nil {
		return fmt.Errorf("failed to remove bind source of %q: %w", mountRequest.Metadata().ID(), err)
	}
