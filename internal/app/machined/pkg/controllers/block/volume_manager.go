...
func (ctrl *VolumeManagerController) Inputs() []controller.Input {
	return []controller.Input{
		...
		{
			Namespace: secrets.NamespaceName,
			Type:      secrets.EncryptionRecoveryKeyType,
			Kind:      controller.InputWeak,
		},
	}
}
...
func (ctrl *VolumeManagerController) Outputs() []controller.Output {
	return []controller.Output{
		...
		{
			// recovery keys are created via the API (on behalf of this controller), and consumed (destroyed) by the volume manager
			Type: secrets.EncryptionRecoveryKeyType,
			Kind: controller.OutputShared,
		},
		{
			// recovery keys generated by the volume manager, fetched (and destroyed) via the API
			Type: secrets.GeneratedRecoveryKeyType,
			Kind: controller.OutputShared,
		},
	}
}
...
func (ctrl *VolumeManagerController) Run(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	for {
		...
		// recovery keys supplied by the operator, they are consumed (destroyed) once used
		recoveryKeys, err := ctrl.listRecoveryKeys(ctx, r)
		if err != nil {
			return err
		}

		usedRecoveryKeys := map[resource.ID]struct{}{}
		...
		for _, vc := range volumeConfigs {
			...
			if err = ctrl.progressVolumeConfig(
				...
				volumes.ManagerContext{
					...
					EncryptionHelpers: encryption.Helpers{
						...
						RecoveryKeyGetter:    ctrl.getRecoveryKeyGetter(recoveryKeys, usedRecoveryKeys),
						RecoveryKeyPublisher: ctrl.getRecoveryKeyPublisher(r),
					},
				...
				if xerrors.TagIs[volumes.Locked](err) {
					// waiting for the operator to supply the recovery key, but keep retrying the automatic keys
					volumeStatus.TypedSpec().Phase = block.VolumePhaseLocked
					shouldRetry = true
				} else {
			...
			_, used := usedRecoveryKeys[vc.Metadata().ID()]

			if used || (recoveryKeys[vc.Metadata().ID()] != nil && volumeStatus.TypedSpec().Phase != block.VolumePhaseLocked) {
				// the recovery key is single-use: it was tried against the volume, or the volume got unlocked without it, don't keep it around
				destroyErr := r.Destroy(ctx, secrets.NewEncryptionRecoveryKey(vc.Metadata().ID()).Metadata())
				if destroyErr != nil && !state.IsNotFoundError(destroyErr) {
					return fmt.Errorf("error destroying used recovery key: %w", destroyErr)
				}
			}
...
func isFailedUserVolumeProvisioning(volumeLifecycleTearingDown bool, volumeConfig *block.VolumeConfig, volumeStatus *block.VolumeStatus) bool {
	if !volumeLifecycleTearingDown || (volumeStatus.TypedSpec().Phase != block.VolumePhaseFailed && volumeStatus.TypedSpec().Phase != block.VolumePhaseLocked) {
		return false
	}
	...
	case block.VolumePhaseWaiting, block.VolumePhaseMissing, block.VolumePhaseLocated,
		block.VolumePhaseProvisioned, block.VolumePhaseFailed, block.VolumePhaseLocked, block.VolumePhaseClosed:
...
func (ctrl *VolumeManagerController) processVolumeConfig(ctx context.Context, logger *zap.Logger, volumeContext volumes.ManagerContext) error {
	...
	for {
		if !volumeContext.ShouldCloseVolume {
			...
			case block.VolumePhaseFailed, block.VolumePhaseLocked:
...
// listRecoveryKeys returns the recovery keys supplied by the operator, indexed by volume ID.
func (ctrl *VolumeManagerController) listRecoveryKeys(ctx context.Context, r controller.Reader) (map[resource.ID]*secrets.EncryptionRecoveryKey, error) {
	list, err := safe.ReaderListAll[*secrets.EncryptionRecoveryKey](ctx, r)
	if err != nil {
		return nil, fmt.Errorf("error fetching recovery keys: %w", err)
	}

	recoveryKeys := map[resource.ID]*secrets.EncryptionRecoveryKey{}

	for key := range list.All() {
		recoveryKeys[key.Metadata().ID()] = key
	}

	return recoveryKeys, nil
}

// getRecoveryKeyGetter returns the recovery key getter for the key handlers.
//
// Every recovery key which was handed out to a key handler is recorded in used, so it can be destroyed afterwards.
func (ctrl *VolumeManagerController) getRecoveryKeyGetter(recoveryKeys map[resource.ID]*secrets.EncryptionRecoveryKey, used map[resource.ID]struct{}) helpers.RecoveryKeyGetter {
	return func(_ context.Context, volumeID string) ([]byte, bool, error) {
		key, ok := recoveryKeys[volumeID]
		if !ok {
			return nil, false, nil
		}

		used[volumeID] = struct{}{}

		return key.TypedSpec().Key, true, nil
	}
}

// getRecoveryKeyPublisher returns the publisher for recovery keys generated by the key handlers.
//
// The generated key is kept in memory (as a sensitive resource) until the operator fetches it via the API.
func (ctrl *VolumeManagerController) getRecoveryKeyPublisher(r controller.Writer) helpers.RecoveryKeyPublisher {
	return func(ctx context.Context, volumeID string, key []byte) error {
		return safe.WriterModify(ctx, r, secrets.NewGeneratedRecoveryKey(volumeID), func(res *secrets.GeneratedRecoveryKey) error {
