...
func LocateAndProvision(logger *zap.Logger, volumeContext ManagerContext) error {
	...
	if !volumeContext.DevicesReady {
		// volume wasn't located, but devices are not ready yet
	...
	case block.VolumeTypePartition:
		...
		partitionCreateResult, err := CreatePartition(logger, pickedDisk, volumeContext.Cfg, diskCheckResult.HasGPT)
		if err != nil {
