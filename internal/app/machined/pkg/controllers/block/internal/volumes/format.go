...
func GrowFilesystem(logger *zap.Logger, volumeContext ManagerContext) error {
	...
	case block.FilesystemTypeXFS:
		...
		manager := mountv3.NewManager(
			...
			mountv3.WithFsopen(volumeContext.Cfg.TypedSpec().Provisioning.FilesystemSpec.Type.String(), fsopen.WithSource(volumeContext.Status.MountLocation), fsopen.WithPrinter(logger.Sugar().Infof)),
		)
