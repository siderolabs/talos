...
func TestParametersChangedNilVsEmpty(t *testing.T) {
	require.False(t, mountconfig.NewSnapshot(nil).ParametersChanged(nil))
	require.False(t, mountconfig.NewSnapshot(nil).ParametersChanged([]block.ParameterSpec{}))
	require.False(t, mountconfig.NewSnapshot([]block.ParameterSpec{}).ParametersChanged(nil))
}
