...
func (suite *DeviceConfigSpecSuite) TestDeviceConfigs() {
	...
	for _, item := range cfgProvider.MachineConfig.MachineNetwork.NetworkInterfaces {
		devices[item.DeviceInterface] = item
	}
		...
		retry.Constant(3*time.Second, retry.WithUnits(100*time.Millisecond)).Retry(
			func() error {
				config, err := suite.state.Get(
					suite.ctx,
					resource.NewMetadata(network.NamespaceName, network.DeviceConfigSpecType, "eth0", resource.VersionUndefined),
				)
				if err != nil {
