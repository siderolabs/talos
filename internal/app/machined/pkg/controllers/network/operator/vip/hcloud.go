...
func (handler *HCloudHandler) Acquire(ctx context.Context) error {
	if handler.networkID > 0 {
		...
		aliase := hcloud.ServerChangeAliasIPsOpts{
		...
		oldDeviceID := findServerByAlise(serverList, handler.networkID, handler.vip)
		if oldDeviceID != 0 {
...
func (handler *HCloudHandler) Release(ctx context.Context) error {
	if handler.networkID > 0 {
		aliace := hcloud.ServerChangeAliasIPsOpts{
			Network:  &hcloud.Network{ID: handler.networkID},
			AliasIPs: []net.IP{},
		}
...
func findServerByAlise(serverList []*hcloud.Server, networkID int, vip string) (deviceID int) {
