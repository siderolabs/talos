...
// raInterval is how often Router Advertisements are emitted on each unnumbered interface.
const raInterval = 10 * time.Second

var allNodesMulticast = netip.MustParseAddr("ff02::1")

// RouterAdvertisementController sends IPv6 Router Advertisements on interfaces used for unnumbered
...
//
// It also enables net.ipv6.conf.<iface>.accept_ra=2 on those interfaces (accept RAs while forwarding)
...
func (ctrl *RouterAdvertisementController) reconcile(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	if cfg != nil {
		for _, bgpConfig := range cfg.Config().NetworkBGPConfigs() {
			for _, neighbor := range bgpConfig.Neighbors() {
				if iface := neighbor.Interface(); iface != "" {
	...
	for _, iface := range sortedKeys(interfaces) {
		...
		if err = safe.WriterModify(ctx, r, runtimeres.NewKernelParamSpec(runtimeres.NamespaceName, id), func(spec *runtimeres.KernelParamSpec) error {
