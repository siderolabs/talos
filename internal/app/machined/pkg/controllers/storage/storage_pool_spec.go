...
func (ctrl *StoragePoolSpecController) Run(ctx context.Context, r controller.Runtime, _ *zap.Logger) error {
	for {
		...
		if cfg != nil {
			for _, doc := range cfg.Config().StoragePoolConfigs() {
				if err = safe.WriterModify(ctx, r, storage.NewStoragePoolSpec(storage.NamespaceName, doc.Name()),
					func(spec *storage.StoragePoolSpec) error {
						spec.TypedSpec().VolumeID = doc.VolumeName()
