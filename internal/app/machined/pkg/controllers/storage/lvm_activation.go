...
// not yet activated, and activates them when the volumes are ready.
//
...
func (ctrl *LVMActivationController) reconcileNewActivations(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	idx, err := ctrl.buildVGDeviceIndex(ctx, r)
	if err != nil {
		return err
	}
	...
	for dv := range discoveredVolumes.All() {
		...
		backingVolumes := idx.backingVolumes(vgName)
		notReady := false

		for _, vs := range backingVolumes {
			switch {
			case vs.TypedSpec().Phase != block.VolumePhaseReady:
				notReady = true
			case vs.Metadata().Phase() == resource.PhaseTearingDown:
				notReady = true
			}
		}

		if notReady {
			// A backing volume is not ready or still in the teardown phase - wait
			logger.Debug("deferring activation, backing volume not ready yet", zap.String("name", vgName))
		...
		for _, vs := range backingVolumes {
			...
			if err := r.AddFinalizer(ctx, vs.Metadata(), finalizer); err != nil {
				multiErr = multierror.Append(multiErr, fmt.Errorf("add finalizer to volume %q: %w", vs.Metadata().ID(), err))
				finalizeErr = true
			}
		}
		...
		if err = ctrl.LVM.VGChangeActivate(ctx, vgName); err != nil {
			...
			multiErr = multierror.Append(multiErr, ctrl.removeFinalizers(ctx, backingVolumes, finalizer, r))
		} else {
...
func (ctrl *LVMActivationController) teardownVG(
	...
	err := ctrl.removeFinalizers(ctx, backingVolumes, finalizer, r)
	if err != nil {
		return err
	}
...
func (ctrl *LVMActivationController) removeFinalizers(ctx context.Context, backingVolumes []*block.VolumeStatus, finalizer string, r controller.Runtime) error {
	for _, vs := range backingVolumes {
		if !vs.Metadata().Finalizers().Has(finalizer) {
			continue
		}
...
// vgDeviceIndex maps VG names to the device paths of their PVs,
// and resolved device paths to the backing block.VolumeStatus.
type vgDeviceIndex struct {
	devicesByVG            map[string][]string
	volumeStatusByLocation map[string]*block.VolumeStatus
}

func (ctrl *LVMActivationController) buildVGDeviceIndex(ctx context.Context, r controller.Reader) (vgDeviceIndex, error) {
	pvStatuses, err := safe.ReaderListAll[*storage.LVMPhysicalVolumeStatus](ctx, r)
	if err != nil {
		return vgDeviceIndex{}, fmt.Errorf("list LVMPhysicalVolumeStatus: %w", err)
	}
	...
	if err != nil {
		return vgDeviceIndex{}, fmt.Errorf("list VolumeStatus: %w", err)
	}
	...
	return vgDeviceIndex{devicesByVG: devicesByVG, volumeStatusByLocation: volumeStatusByLocation}, nil
}

// backingVolumes returns the VolumeStatus resources backing the given VG's
// known physical volumes.
func (idx vgDeviceIndex) backingVolumes(vgName string) []*block.VolumeStatus {
	seen := map[string]struct{}{}
	result := make([]*block.VolumeStatus, 0, len(idx.devicesByVG[vgName]))

	for _, device := range idx.devicesByVG[vgName] {
		vs, ok := idx.volumeStatusByLocation[resolveDevicePath(device)]
		if !ok {
			continue
		}
