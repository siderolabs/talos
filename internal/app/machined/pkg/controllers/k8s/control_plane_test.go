...
func (suite *K8sControlPlaneSuite) TestReconcileCustomClusterDNS() {
	u, err := url.Parse("https://foo:6443")
	suite.Require().NoError(err)
	...
	v1alpha1Cfg := &v1alpha1.Config{
		...
		MachineConfig: &v1alpha1.MachineConfig{
			...
			MachineKubelet: &v1alpha1.KubeletConfig{
				KubeletClusterDNS: []string{"10.96.0.42"},
			},
		},
		ClusterConfig: &v1alpha1.ClusterConfig{
			ControlPlane: &v1alpha1.ControlPlaneConfig{
	...
	rtestutils.AssertResources(
		suite.Ctx(), suite.T(), suite.State(), []resource.ID{k8s.BootstrapManifestsConfigID},
		func(bootstrapConfig *k8s.BootstrapManifestsConfig, assert *assert.Assertions) {
			// the kube-dns Service must be generated with the custom clusterDNS address,
			// not the default service-CIDR-derived one (https://github.com/siderolabs/talos/issues/12618)
			assert.Equal("10.96.0.42", bootstrapConfig.TypedSpec().DNSServiceIP)
			assert.Equal("", bootstrapConfig.TypedSpec().DNSServiceIPv6)
		},
	)
}

func (suite *K8sControlPlaneSuite) TestReconcileCustomClusterDNSDualStack() {
	u, err := url.Parse("https://foo:6443")
	suite.Require().NoError(err)
	...
	v1alpha1Cfg := &v1alpha1.Config{
		...
		MachineConfig: &v1alpha1.MachineConfig{
			MachineType: "controlplane",
			MachineKubelet: &v1alpha1.KubeletConfig{
				KubeletClusterDNS: []string{"10.96.0.42", "fd00::42"},
			},
		},
		ClusterConfig: &v1alpha1.ClusterConfig{
			ControlPlane: &v1alpha1.ControlPlaneConfig{
	...
	rtestutils.AssertResources(
		...
		func(bootstrapConfig *k8s.BootstrapManifestsConfig, assert *assert.Assertions) {
			assert.Equal("10.96.0.42", bootstrapConfig.TypedSpec().DNSServiceIP)
			assert.Equal("fd00::42", bootstrapConfig.TypedSpec().DNSServiceIPv6)
		},
	)
}
