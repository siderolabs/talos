...
func (ctrl *ManifestApplyController) applyManifests(
	...
	if err = etcd.WithLock(ctx, constants.EtcdTalosManifestApplyMutex, logger, func() error {
		inv, err := ssa.GetInventory(ctx, dyn, mapper, constants.KubernetesInventoryNamespace, constants.KubernetesBootstrapManifestsInventoryName)
		if err != nil {
			return fmt.Errorf("error getting inventory: %w", err)
		}

		inventoryContents, err := inv.Read(ctx)
		if err != nil {
		...
		err = inv.Write(ctx, inventoryContents)
		if err != nil {
