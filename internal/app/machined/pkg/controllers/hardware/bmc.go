...
func (ctrl *BMCDevicesController) Run(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	for {
		...
		for _, path := range paths {
			devnum, isDevice := ipmi.DeviceNumber(path)
			if !isDevice {
				continue
			}
			...
			if !ok {
				if spec, err = ctrl.query(ctx, logger, path, devnum); err != nil {
...
func (ctrl *BMCDevicesController) query(ctx context.Context, logger *zap.Logger, path string, devnum int32) (hardware.BMCDeviceSpec, error) {
	dev, err := ipmi.Open(ctx, devnum)
	if err != nil {
		return hardware.BMCDeviceSpec{}, err
	}

	defer dev.Close(ctx) //nolint:errcheck

	deviceInfo, err := ipmi.DeviceID(ctx, dev)
	if err != nil {
		return hardware.BMCDeviceSpec{}, err
	}
	...
	lanConfig, err := ipmi.FindLANConfig(ctx, dev.GetLanConfigParamFor)
	if err != nil {
		if ctx.Err() != nil {
			return hardware.BMCDeviceSpec{}, err
		}
