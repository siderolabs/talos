...
func TestServiceConditionDeadlineErrorMessage(t *testing.T) {
	...
	// cancellation keeps returning the bare context.Canceled: the exact match
	// is relied upon by conditions.WaitForAll's collapse
	cancelCtx, cancel := context.WithCancel(context.Background())
	cancel()

	err = system.WaitForServiceAnyEventWithInstance(services, upOrFinished(), "gated-service").Wait(cancelCtx)
