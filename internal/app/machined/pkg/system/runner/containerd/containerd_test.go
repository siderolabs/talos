...
func (suite *ContainerdSuite) assertRunSucceeds(r runner.Runner) {
...
func (suite *ContainerdSuite) SetupSuite() {
	...
	// cri-containerd.toml (constants.CRIContainerdConfig) hard-imports /etc/cri/conf.d/cri.toml,
	// which on a real node is generated by CRIConfigController. It is not shipped in the
	// rootfs, so provide an empty one here — containerd v2 fatals on a missing literal import.
	suite.Require().NoError(os.MkdirAll(constants.EtcCRIConfdPath, 0o755))
	suite.Require().NoError(os.WriteFile(filepath.Join(constants.EtcCRIConfdPath, "cri.toml"), nil, 0o644))
