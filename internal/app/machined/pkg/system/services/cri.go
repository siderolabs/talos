...
func (c *CRI) Runner(r runtime.Runtime) (runner.Runner, error) {
	...
	// On bare-metal/VM, run CRI inside the shared sandbox PID+mount namespace.
	// The launcher is resolved per launch (getter), so if sandboxd is
	// recreated, CRI's restart re-enters the new namespace. In container mode the
	// sandboxd service is not loaded, so CRI runs on the host as before.
