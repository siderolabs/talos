...
func PartitionOp(
	...
	mountOptions []mount.MountOption,
) error {
		...
		for _, partition := range info.Parts {
			if pointer.SafeDeref(partition.PartitionLabel) == spec.PartitionLabel {
				if partition.Name != spec.FilesystemType {
					return xerrors.NewTaggedf[NotFoundTag]("partition with label %s is not of type %s (actual %s)", *partition.PartitionLabel, spec.FilesystemType, partition.Name)
				}
