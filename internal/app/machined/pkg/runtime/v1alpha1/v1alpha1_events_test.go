	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.watchers; i++ {
				if err := e.Watch(func(events <-chan runtime.IncomingEvent) {
					defer wg.Done()
...
func receive(t *testing.T, e runtime.Watcher, n int, opts ...runtime.WatchOptionFunc) (result []runtime.IncomingEvent) {
	var wg sync.WaitGroup
...
func extractSeq(t *testing.T, events []runtime.IncomingEvent) (result []int) {
...
func BenchmarkWatch(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = e.Watch(func(events <-chan runtime.IncomingEvent) { wg.Done() }) //nolint:errcheck
	}
	...
	for _, watchers := range []int{0, 1, 10} {
		bb.Run(fmt.Sprintf("Watchers-%d", watchers), func(b *testing.B) {
			...
			for j := 0; j < watchers; j++ {
				_ = e.Watch(func(events <-chan runtime.IncomingEvent) { //nolint:errcheck
					defer wg.Done()
