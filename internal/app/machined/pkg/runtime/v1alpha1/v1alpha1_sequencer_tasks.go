...
// WipeStagedVolumes wipes the system volumes staged for wiping via the StagedVolumeWipe META tag.
//
// It's driven by the StagedVolumeWipe META tag (written by the VolumeWipe API with on_reboot=true),
// runs early in the Initialize sequence before volumes are provisioned, clears the tag, wipes the
// staged volumes, and reboots so the volume manager re-provisions from a clean GPT.
func WipeStagedVolumes(seq runtime.Sequence, _ any) (runtime.TaskExecutionFunc, string) {
	reboot, _ := Reboot(seq, nil)

	return func(ctx context.Context, logger *log.Logger, r runtime.Runtime) error {
		tagValue, ok := r.State().Machine().Meta().ReadTag(metamachinery.StagedVolumesToWipe)
		if !ok {
			return errors.New("no staged volume wipe tag found")
		}

		// clear the tag before wiping/rebooting so a wipe failure can't cause a boot loop
		if _, err := r.State().Machine().Meta().DeleteTag(ctx, metamachinery.StagedVolumesToWipe); err != nil {
			return fmt.Errorf("failed to delete staged volume wipe tag: %w", err)
		}
		...
		if err := json.Unmarshal([]byte(tagValue), &volumeIDs); err != nil {
			return fmt.Errorf("failed to decode staged volume wipe tag: %w", err)
		}

		targets, missing, err := resolveStagedVolumeWipeTargets(ctx, r.State().V1Alpha2().Resources(), volumeIDs)
		if err != nil {
			return err
		}

		for _, id := range missing {
			logger.Printf("skipping staged wipe of volume %q: not found", id)
		}

		var wipeErrors *multierror.Error

		for _, target := range targets {
			logger.Printf("wiping staged volume %s", target)

			if err := target.Wipe(ctx, logger.Printf); err != nil {
				wipeErrors = multierror.Append(wipeErrors, fmt.Errorf("failed wiping volume %s: %w", target, err))
			}
		}
		...
		logger.Printf("finished wiping staged volumes, rebooting")

		return reboot(ctx, logger, r)
	}, "wipeStagedVolumes"
}

// resolveStagedVolumeWipeTargets resolves staged volume IDs to wipe targets.
//
// In the early boot phase we don't have VolumeStatus resources yet, so DiscoveredVolumes are used to
// resolve the volume IDs (== partition labels for system volumes) to wipe targets. Volume IDs which
// don't match any discovered volume are returned in missing rather than failing the whole wipe.
func resolveStagedVolumeWipeTargets(ctx context.Context, st state.CoreState, volumeIDs []string) (targets []*partition.VolumeWipeTarget, missing []string, err error) {
	discoveredVolumes, err := safe.StateListAll[*blockres.DiscoveredVolume](ctx, st)
	if err != nil {
		return nil, nil, err
	}

	for _, id := range volumeIDs {
		var target *partition.VolumeWipeTarget

		for discoveredVolume := range discoveredVolumes.All() {
			if discoveredVolume.TypedSpec().PartitionLabel == id {
				target = partition.VolumeWipeTargetFromDiscoveredVolume(discoveredVolume)
