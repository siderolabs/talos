...
func (n *Nocloud) configFromCD() (metaConfig []byte, networkConfig []byte, machineConfig []byte, err error) {
	...
	dev, err = probe.GetDevWithFileSystemLabel(strings.ToLower(configISOLable))
	if err != nil {
		dev, err = probe.GetDevWithFileSystemLabel(strings.ToUpper(configISOLable))
		if err != nil {
