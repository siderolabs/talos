...
// dualStackOptions alternates download attempts between the IPv4 and IPv6 endpoints, so an
// instance reaches the metadata service over whichever address family it actually has. Both
// serve the same content, so whichever answers first is authoritative. Retries are driven by
// the download package, which calls the endpoint function once per attempt.
func dualStackOptions(ipv4Endpoint, ipv6Endpoint string) []download.Option {
...
func (u *Scaleway) getMetadata(ctx context.Context) (*instance.Metadata, error) {
	metaConfigDl, err := download.Download(ctx, ScalewayMetadataEndpoint,
		dualStackOptions(ScalewayMetadataEndpoint, ScalewayMetadataEndpointIPv6)...)
	if err != nil {
