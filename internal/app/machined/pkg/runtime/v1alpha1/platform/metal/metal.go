...
func (m *Metal) Configuration() ([]byte, error) {
	...
	switch *option {
	case universe.MetalConfigISOLabel:
...
func readConfigFromISO() (b []byte, err error) {
	...
	dev, err = probe.GetDevWithFileSystemLabel(universe.MetalConfigISOLabel)
	if err != nil {
		return nil, fmt.Errorf("failed to find %s iso: %w", universe.MetalConfigISOLabel, err)
	}
	...
	b, err = ioutil.ReadFile(filepath.Join(mnt, filepath.Base(universe.ConfigPath)))
	if err != nil {
