...
func TestSystemVolumeStatuses(t *testing.T) {
	...
	// A wipe zeroes a partition's superblocks and the volume manager then re-provisions it; a volume
	// which is not partition-backed has nothing of the sort to wipe.
	t.Run("non-partition-backed system volume", func(t *testing.T) {
		dirVol := block.NewVolumeStatus(block.NamespaceName, "DIRECTORY-VOLUME")
		dirVol.Metadata().Labels().Set(block.SystemVolumeLabel, "")
		dirVol.TypedSpec().Type = block.VolumeTypeDirectory
		require.NoError(t, st.Create(ctx, dirVol))

		_, err := system.ResolveSystemVolumeStatuses(ctx, st, []string{"DIRECTORY-VOLUME"})
		require.Error(t, err)
		assert.Equal(t, codes.InvalidArgument, grpcstatus.Code(err))
		assert.Contains(t, err.Error(), "not a partition-backed volume")
	})
