...
FROM base AS osctl-linux-arm64-build
ARG SHA
ARG TAG
ARG VERSION_PKG="github.com/talos-systems/talos/internal/pkg/version"
WORKDIR /src/cmd/osctl
RUN GOOS=linux GOARCH=arm64 CGO_ENABLED=1 go build -a -ldflags "-s -w -linkmode external -extldflags \"-static\" -X ${VERSION_PKG}.Name=Client -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /osctl-linux-arm64
RUN chmod +x /osctl-linux-arm64

FROM scratch AS osctl-linux-arm64
COPY --from=osctl-linux-arm64-build /osctl-linux-arm64 /osctl-linux-arm64
...
COPY --from=ntpd / /
COPY --from=udevd / /
COPY --from=osctl-linux-arm64 / /
COPY --from=osctl-darwin-amd64 / /

FROM scratch AS binaries
COPY --from=binaries-build /osctl-linux-arm64 /osctl-linux-arm64
...
COPY --from=initramfs-build /bin/extlinux /bin
COPY --from=osctl-linux-arm64-build /osctl-linux-arm64 /bin/osctl
RUN curl -L https://releases.hashicorp.com/packer/1.3.1/packer_1.3.1_linux_arm64.zip -o /tmp/packer.zip \
