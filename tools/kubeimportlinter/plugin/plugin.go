...
// Package plugin exposes kubeimportlinter as a golangci-lint module plugin.
package plugin
...
import (
	...
	"github.com/siderolabs/talos/tools/kubeimportlinter/internal/kubeimportlinter"
)

// Name is the registered golangci-lint linter name for this module plugin.
const Name = kubeimportlinter.AnalyzerName
...
type Settings struct {
	...
	Rules   kubeimportlinter.Rules `json:"rules"`
}

// Linter adapts kubeimportlinter to the golangci-lint module plugin interface.
type Linter struct {
	config kubeimportlinter.Config
}
...
func New(conf any) (register.LinterPlugin, error) {
	...
	cfg, err := kubeimportlinter.NormalizeConfig(kubeimportlinter.Config{
...
func (l *Linter) BuildAnalyzers() ([]*analysis.Analyzer, error) {
	return []*analysis.Analyzer{kubeimportlinter.NewAnalyzer(l.config)}, nil
}
...
func detectConfigBaseDir() (string, error) {
	...
	if dir, _, ok := kubeimportlinter.SearchUpward(cwd, ".golangci.yml", ".golangci.yaml", ".golangci.toml", ".golangci.json"); ok {
		return dir, nil
	}
