...
LABEL Dianemo
  ...
  APPEND ip=dhcp console=tty1 console=ttyS0 dianemo.autonomy.io/root=/dev/sda
EOF
...
function raw() {
  ...
  parted -s -a none /dianemo.raw mkpart ROOT xfs 50M 2G
...
function rootfs() {
  dd if=/dev/zero of=/rootfs.raw bs=1M count=650
